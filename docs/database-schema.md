...
The SQL is available at [001_initial_audit_log_schema.sql](../database/001_initial_audit_log_schema.sql). It targets PostgreSQL 16+ and is a reference migration, not an application implementation.
...
| `chain_sequence` | BIGINT identity | Unique, monotonic event order and pagination cursor. It is database-assigned and cannot be null. |
...
| `recorded_at` | TIMESTAMPTZ | Required server-assigned UTC audit time. |
...
`chain_sequence` has a uniqueness constraint. Hash columns must match lower-case 64-character hexadecimal form. The first-link check catches a malformed genesis link, but the repository transaction and verification service enforce that every later `previous_hash` exactly equals the immediately preceding event's `content_hash`.
...
| `encryption_nonce` | BYTEA | Required nonce/IV used for encryption. |
| `ciphertext` | BYTEA | Required encrypted canonical JSON payload. |
| `stored_at` | TIMESTAMPTZ | Time encrypted material was persisted. |
...
| `updated_at` | TIMESTAMPTZ | Last successful append coordination update. |
...
| `signature` | BYTEA | Signature over the canonical checkpoint record. |
| `anchored_at` | TIMESTAMPTZ | Creation time. |
...
| `accepted_at` | TIMESTAMPTZ | Request acceptance time. |
...
| `archived_by`, `archived_at` | VARCHAR / TIMESTAMPTZ | Accountable actor and time. |
...
| `authorized_by`, `redacted_at` | VARCHAR / TIMESTAMPTZ | Accountable authorization evidence. |
...
The assignment permits filters in any combination. A separate compound index for every possible combination would be wasteful. PostgreSQL can combine selective indexes with bitmap scans; query plans should be checked against realistic data. If workload evidence shows a frequent combination, add a targeted index in a later migration. Every primary query index ends in `chain_sequence`, enabling keyset pagination without offset-scan degradation.
