...
public class RetentionArchiveService {
    ...
    public ArchiveManifestEntity archiveEligible(String archivedBy) {
        ...
        return manifests.save(new ArchiveManifestEntity(UUID.randomUUID(), first.getChainSequence(), last.getChainSequence(), first.getPreviousHash(), first.getContentHash(), last.getContentHash(), retention.archiveUriPrefix()+"/"+first.getChainSequence()+"-"+last.getChainSequence(), bundleHash, "v1", archivedBy, now));
    }
}
